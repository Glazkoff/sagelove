        ...
        for user in users_all:
            ...
            for user_second in users:
                ...
                for question_with_option in questions_with_option:
                    ...
                    second_user_answer_option = UserOptionAnswer.objects.filter(
                        user=user_second, question_with_option=question_with_option).first()
...
def second_algorithm():
    try:
        ...
        for user in users_all:
            ...
            count_match = 0
            for user_second in users:
                first_user_answer_scale_1 = UserScaleAnswer.objects.filter(
                    user=user_first, answer=1).count()
                second_user_answer_scale_1 = UserScaleAnswer.objects.filter(
                    user=user_second, answer=1).count()
                first_user_answer_scale_5 = UserScaleAnswer.objects.filter(
                    user=user_first, answer=5).count()
                second_user_answer_scale_5 = UserScaleAnswer.objects.filter(
                    user=user_second, answer=5).count()
                first_user_answer_scale_2 = UserScaleAnswer.objects.filter(
                    user=user_first, answer=2).count()
                second_user_answer_scale_2 = UserScaleAnswer.objects.filter(
                    user=user_second, answer=2).count()
                first_user_answer_scale_4 = UserScaleAnswer.objects.filter(
                    user=user_first, answer=4).count()
                second_user_answer_scale_4 = UserScaleAnswer.objects.filter(
                    user=user_second, answer=4).count()
                if Datings.objects.all().filter(
                    (
                        Q(user_1=first_user_data)
                        & Q(user_2=user_second)
